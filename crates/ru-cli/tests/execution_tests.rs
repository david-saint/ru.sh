...
async fn test_e2e_execution_flow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    .arg("--yes")
    .arg("--test-allow-non-interactive-yes")
    .arg("--shell")
    .arg("bash");
...
#[tokio::test]
async fn test_ru_test_mode_env_does_not_bypass_non_interactive_yes_guard()
-> Result<(), Box<dyn std::error::Error>> {
    let mock_server = MockServer::start().await;
    let tmp_dir = tempdir()?;
    let blocked_file = tmp_dir.path().join("blocked.txt");
    let blocked_file_str = blocked_file.to_str().unwrap();
        ...
        "choices": [
            {
                "message": {
                    "role": "assistant",
                    "content": format!("echo 'should-not-run' > '{}'", blocked_file_str)
                }
            }
        ]
    });
    ...
    assert!(
        !blocked_file.exists(),
        "command should not execute in non-interactive mode"
    );
